...
document.addEventListener("DOMContentLoaded", function () {
    ...
    window.addEventListener("pageshow", function (event) {
        if (event.persisted || performance.getEntriesByType("navigation")[0].type === "back_forward") {
            ...
            window.location.href = "https://www.rootairs.com/main/";
        }
    });
    ...
    fetch(`https://www.rootairs.com/api/pay/pay_data_common?flight_id=${flightId}`, {
        method: "GET",
        credentials: "include"
    })
    .then(response => response.json())
    ...
    async function fetchMileage() {
        try {
            let response = await fetch("https://www.rootairs.com/api/pay/get_mileage", {
                method: "GET",
                credentials: "include"
            });
    ...
    document.getElementById("pay-button").addEventListener("click", async function () { 
        ...
        let userId, username;
        try {
            let response = await fetch("https://www.rootairs.com/api/member/status", {
                method: "GET",
                credentials: "include"
            });
        ...
        let paymentUrl = `https://www.rootairs.com/pay/pay_info?${queryParams.toString()}`;
    ...
    async function processInicisPayment(amount) {  
        ...
        try {
            ...
            IMP.request_pay({
                ...
                if (rsp.success) {
                    try {
                        let paymentResponse = await fetch("https://www.rootairs.com/api/pay/process_inicis_payment", {
                            method: "POST",
                            credentials: "include",
