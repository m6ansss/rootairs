document.addEventListener("DOMContentLoaded", function () {
    ...
    function updateNavbar() {
		...
		const adminUrl = data.admin_url;
                navbarMember.innerHTML = data.is_admin
                    ? `<li><a href="https://www.rootairs.com/api/member/logout">로그아웃</a></li>
                       <li><a href="${adminUrl}">회원정보</a></li>`
                    : `<li><a href="https://www.rootairs.com/api/member/logout">로그아웃</a></li>
                       <li><a href="https://www.rootairs.com/mypage/mypage.html">마이페이지</a></li>`;
            } else {
                navbarMember.innerHTML = `<li><a href="https://www.rootairs.com/member/member_signup.html">회원가입</a></li>
                                          <li><a href="https://www.rootairs.com/member/member_login.html">로그인</a></li>`;
            }
        })
    ...
    if (verifiedEmail) {
        ...
        alert("이메일 인증이 필요합니다.");
        window.location.href = "https://www.rootairs.com/member/member_email.html";
    }

    // /** ✅ 이메일 인증 후 자동 입력 */
    // fetch("https://www.rootairs.com/api/member/get_verified_email", {
    //     method: "GET",
    //     credentials: "include"
    // })
    // .then(response => response.json())
    ...
    function checkId() {
        ...
        fetch("https://www.rootairs.com/api/member/check-id", {
            method: "POST",
            credentials: "include", // ✅ 쿠키 포함
    ...
    signupButton.addEventListener("click", function (event) {
        ...
        fetch("https://www.rootairs.com/api/member/signup", {
            method: "POST",
            credentials: "include", // ✅ 쿠키 포함
            headers: { "Content-Type": "application/json" },
   ...
   // validateField(passwordInput, /^(?=.*[a-zA-Z])(?=.*\d)(?=.*[!@#$%^&*])[A-Za-z\d!@#$%^&*]{8,20}$/, passwordError, "비밀번호는 영문, 숫자, 특수문자를 포함한 8~20자로 설정해야 합니다.");
    ...
    document.getElementById("logoutButton").addEventListener("click", function () {
        fetch("https://www.rootairs.com/api/member/logout", {
            method: "POST",
            credentials: "include"
        })
        .then(response => response.json())
        .then(data => {
            alert(data.message);
            window.location.href = "https://www.rootairs.com/main/main.html";
        })
