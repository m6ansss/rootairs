...
function fetchNotices(page = 1) {
    ...
    fetch(`http://www.rootairs.com/api/notices/list?page=${page}`)
...
function displayNotices(notices) {
    ...
    notices.forEach(notice => {
        ...
        row.innerHTML = `
            <td>📢 공지사항</td>
            <td><a href="http://www.rootairs.com:80/notices/notice_detail.html?notice_id=${notice.notice_id}" class="notice_link">
                ${notice.title}
            </a></td>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.getElementById("nextPage").addEventListener("click", () => {
        fetch(`http://www.rootairs.com/api/notices/list?page=${currentPage + 1}`)
            .then(response => response.json())
            .then(data => {
