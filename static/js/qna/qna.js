...
function fetchInquiryList(page = 1) {
    ...
    fetch(`http://www.rootairs.com/api/qna/?page=${page}`, {
	method: "GET",
	credentials:"include"
    })
        .then(response => response.json())
...
function fetchMyInquiryList(page = 1) {
   // console.log(🔥 [DEBUG] 나의 문의 API 호출: 페이지 = ${page});
    ...
    fetch(`http://www.rootairs.com/api/qna/my?page=${page}`, {
       method: "GET",
       credentials: "include"
    })
    .then(response => response.json())
...
function fetchCurrentUser() {
    ...
    fetch('http://www.rootairs.com/api/qna/',{  // ✅ qna_api() 호출하면 user_id 확인 가능
        method: "GET",
        credentials: "include"
	})
        .then(response => response.json())
