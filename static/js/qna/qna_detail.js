...
function loadQnaDetail() {
    ...
    fetch(`http://www.rootairs.com/api/qna/detail/${qnaId}`, {
        method: "GET",
        credentials: "include"
    })
    .then(response => response.json())
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // 관리자 확인 (기존 코드)
    fetch("http://www.rootairs.com/api/member/status", {  // 관리자 여부 체크
            method: "GET",
            credentials: "include"
        })
        .then(response => response.json())
    ...
    function loadComment(qnaId) {
        fetch(`http://www.rootairs.com/api/qna/${qnaId}`)  // 댓글을 불러오는 GET 요청
            .then(response => response.json())
            .then(data => {
                if (data.comment) {
    ...
    if (answerBtn) {
        answerBtn.addEventListener("click", function () {
            ...
            fetch(`http://www.rootairs.com/api/qna/comment/${qnaId}`, {
                method: "POST",
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (deleteBtn) {
        deleteBtn.addEventListener("click", function () {
            ...
            fetch(`http://www.rootairs.com/api/qna/delete/${qnaId}`, {
                method: "DELETE"
            })
            .then(response => response.json())
            .then(data => {
                if (data.message) {
                    alert(data.message);
		   window.location.href = "http://www.rootairs.com:80/qna/qna.html";
                } else {
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (backBtn) {
        backBtn.addEventListener("click", function () {
            window.location.href = "http://www.rootairs.com:80/qna/qna.html";  // ✅ 목록 페이지 URL로 변경
        });
    }
});
