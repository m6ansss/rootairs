...
function loadQnaDetail() {
    ...
    fetch(`https://www.rootairs.com/api/qna/detail/${qnaId}`, {
        method: "GET",
        credentials: "include"
    })
    .then(response => response.json())
    .then(data => {
        ...
        document.getElementById("qna_content").innerHTML = data.content;
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // 관리자 확인 (기존 코드)
    fetch("https://www.rootairs.com/api/member/status", {  // 관리자 여부 체크
            method: "GET",
            credentials: "include"
        })
        .then(response => response.json())
    ...
    function loadComment(qnaId) {
        fetch(`https://www.rootairs.com/api/qna/${qnaId}`)  // 댓글을 불러오는 GET 요청
            .then(response => response.json())
            .then(data => {
                if (data.comment) {
    ...
    if (answerBtn) {
        answerBtn.addEventListener("click", function () {
            ...
            fetch(`https://www.rootairs.com/api/qna/comment/${qnaId}`, {
                method: "POST",
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (deleteBtn) {
        deleteBtn.addEventListener("click", function () {
            ...
            fetch(`https://www.rootairs.com/api/qna/delete/${qnaId}`, {
                method: "POST",
                credentials: "include"
            })
            .then(response => response.json())
            .then(data => {
                if (data.message) {
                    alert(data.message);
		   window.location.href = "https://www.rootairs.com/qna/qna.html";
                } else {
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (backBtn) {
        backBtn.addEventListener("click", function () {
            window.location.href = "https://www.rootairs.com/qna/qna.html";  // ✅ 목록 페이지 URL로 변경
        });
    }
});
